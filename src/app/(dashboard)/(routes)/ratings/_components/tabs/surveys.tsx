...
const SurveyRatingsTab = () => {
	const { token } = useToken();

	const { data, isPending, error, refetch } = useQuery({
		queryKey: ["Rating Overview", { type: "s" }],
		queryFn: () => ratingService.getOverview("s", token || ""),
		enabled: !!token,
	});
	...
	return (
		<>
			...
			<section className="mb-12 mt-6 grid grid-cols-1 px-6">
				<ActivitiesRatedCard type="survey" data={data.data?.activitiesRated} />
			</section>
		</>
	);
};

export default SurveyRatingsTab;
