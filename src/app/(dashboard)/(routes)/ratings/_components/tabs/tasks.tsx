...
const TaskRatingsTab = () => {
	const { token } = useToken();

	const { data, isPending, error, refetch } = useQuery({
		queryKey: ["Rating Overview", { type: "t" }],
		queryFn: () => ratingService.getOverview("t", token || ""),
		enabled: !!token,
	});
	...
	return (
		<>
			...
			<section className="mb-12 mt-6 grid grid-cols-1 px-6">
				<ActivitiesRatedCard type="task" data={data.data?.activitiesRated} />
			</section>
		</>
	);
};

export default TaskRatingsTab;
